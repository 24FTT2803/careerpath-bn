...
@section('title', 'Create Proficiency Level')

@section('content')
<div class="biicf-page">
    <div class="page-header">
        <div>
            <h1><i class="fas fa-plus-circle"></i> Create Proficiency Level</h1>
            <p class="text-muted">Add a new proficiency level to the BIICF framework</p>
        </div>
    ...
    <div class="card" style="max-width: 800px;">
        <div class="card-body">
            <form method="POST" action="{{ route('admin.biicf.proficiency-levels.store') }}">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select name="level_number" class="form-control @error('level_number') is-invalid @enderror" required>
                        ...
                        <option value="1" {{ old('level_number') == 1 ? 'selected' : '' }}>Level 1 - Follow</option>
                        <option value="2" {{ old('level_number') == 2 ? 'selected' : '' }}>Level 2 - Assist</option>
                        <option value="3" {{ old('level_number') == 3 ? 'selected' : '' }}>Level 3 - Apply</option>
                        <option value="4" {{ old('level_number') == 4 ? 'selected' : '' }}>Level 4 - Ensure</option>
                        <option value="5" {{ old('level_number') == 5 ? 'selected' : '' }}>Level 5 - Strategise</option>
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="name" class="form-control @error('name') is-invalid @enderror" 
                           value="{{ old('name') }}" placeholder="e.g. Follow, Assist, Apply, Ensure, Strategise" required>
                    @error('name')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="description" class="form-control @error('description') is-invalid @enderror" 
                              rows="3" placeholder="Describe this proficiency level...">{{ old('description') }}</textarea>
                    @error('description')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="form-actions">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Create Proficiency Level
                    </button>
                </div>
            </form>
        </div>
