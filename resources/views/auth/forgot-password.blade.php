...
<head>
    ...
    <style>
        ...
        *,*::before,*::after{box-sizing:border-box;margin:0;padding:0}
        ...
        .auth-card h1{font-family:var(--font-display);font-size:26px;font-weight:600;margin-bottom:12px}
        .auth-card .lede{color:var(--paper-dim);font-size:14.5px;margin-bottom:26px;line-height:1.6}
        ...
        .btn-gold{background:var(--gold);color:var(--ink);width:100%;margin-top:6px}
...
<body>
    ...
    <main class="auth-main">
        <div class="auth-card reveal">
            ...
            <p class="lede">{{ __('No problem. Enter your email and we\'ll send you a link to choose a new one.') }}</p>
            ...
            <form method="POST" action="{{ route('password.email') }}">
                ...
                <div class="field">
                    ...
                    <input id="email" type="email" name="email" value="{{ old('email') }}" placeholder="you@pb.edu.bn" required autofocus>
                    @error('email')<div class="err">{{ $message }}</div>@enderror
                </div>

                <button type="submit" class="btn btn-gold">Email password reset link</button>

                <div class="form-foot">
                    Remembered it after all? <a href="{{ route('login') }}">Log in</a>
                </div>
            </form>
        </div>
    </main>
